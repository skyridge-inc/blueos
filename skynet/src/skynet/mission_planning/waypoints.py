...
def read_waypoints(
    path: str, include_home: bool = False
) -> list[tuple[float, float]]:
    """Parse a QGC WPL 110 file, returning waypoints as (lat, lon) tuples.

    When include_home=True, the home row (index 0) is returned as the first
    element so the result is indexed exactly as the MAVLink mission protocol
    expects: seq 0 = home, seq 1..N = mission items.
    """
    home: tuple[float, float] | None = None
    waypoints: list[tuple[float, float]] = []
    with open(path) as f:
